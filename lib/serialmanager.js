...
const AUTO_MANUFACTURERS = [ 'FTDI', 'Adafruit' ];
...
class SerialManager {
  ...
  constructor(options) {
    ...
    openSerialPort(path, (err, serialPort, path) => {
      if(err) {
        return console.log('neopixel-serial-bindicators',
  ...
  write(data, callback) {
    ...
    if(!self.serialPort) {
      return callback(new Error('Serial port is not open/available'));
    }
...
function openSerialPort(path, callback) {
  ...
  if(path === AUTO_PATH) {
    ...
    SerialPort.list().then(ports => {
      ports.forEach(port => {
        if(AUTO_MANUFACTURERS.includes(port.manufacturer)) {
          serialPort = new SerialPort({ path: port.path, baudRate: BAUDRATE },
                                      (err) => {
            console.log('neopixel-serial-bindicators: auto serial path: \"' +
                        port.path + '\" was selected');
            return callback(err, serialPort, port.path);
          });
        }
        else if(port.manufacturer) {
          console.log('neopixel-serial-bindicators: alternate serial path: \"' +
